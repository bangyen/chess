...
for name, fen in test_positions:
    ...
    print(
        f"  Material (us/them): {features['material_us']:.1f} / {features['material_them']:.1f}"
    )
    print(
        f"  Mobility (us/them): {features['mobility_us']:.1f} / {features['mobility_them']:.1f}"
    )
    print(f"  Phase: {features['phase']:.0f}")
    if "king_ring_pressure_us" in features:
        print(
            f"  King ring pressure (us/them): {features['king_ring_pressure_us']:.1f} / {features['king_ring_pressure_them']:.1f}"
        )
