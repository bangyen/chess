...
def parse_metrics(output: str) -> Dict[str, float]:
    ...
    if m: metrics['tau'] = float(m.group(1))

    # Local faithfulness (best vs 2): 87.5%
    m = re.search(r"Local faithfulness \(best vs 2\):\s+([0-9.]+)%", output)
    if m: metrics['faithfulness'] = float(m.group(1))

    # Sparsity (reasons...): 1.25
    m = re.search(r"Sparsity .*:\s+([0-9.]+)", output)
    if m: metrics['sparsity'] = float(m.group(1))

    # Coverage (>=2 strong reasons):  93.8%
    m = re.search(r"Coverage .*:\s+([0-9.]+)%", output)
    if m: metrics['coverage'] = float(m.group(1))

    return metrics

def main():
    parser = argparse.ArgumentParser(description="Benchmark chess-ai-audit across two commits")
    ...
    parser.add_argument("--positions", type=int, default=400, help="Number of positions to test")
    parser.add_argument("--seed", type=int, default=42, help="Random seed for reproducibility")
    parser.add_argument("--engine", default=run_command("which stockfish").stdout.strip() or "/opt/homebrew/bin/stockfish", help="Path to Stockfish engine")
