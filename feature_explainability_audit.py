...
try:
    ...
    print("tqdm is required for progress bars. Install with: pip install tqdm", file=sys.stderr)
    raise
...
try:
    ...
    print("python-chess is required. Install with: pip install python-chess", file=sys.stderr)
    raise
...
def load_feature_module(path: str):
    ...
    if not hasattr(mod, "extract_features"):
        raise RuntimeError("Feature module must define extract_features(board) -> Dict[str, number|bool]")
    return mod
...
def baseline_extract_features(board: "chess.Board") -> Dict[str, float]:
    ...
    def file_state(side):
        ...
        semi_open = 0
        for file in range(8):
            pawns = [sq for sq in chess.SQUARES if chess.square_file(sq)==file and board.piece_type_at(sq)==chess.PAWN]
    ...
    feats["phase"] = float(sum(len(board.pieces(pt, True))+len(board.pieces(pt, False)) for pt in [chess.QUEEN,chess.ROOK,chess.BISHOP,chess.KNIGHT]))
    ...
    def hanging_pieces(side):
        ...
        for sq in chess.SQUARES:
            piece = board.piece_at(sq)
            if piece and piece.color == side:
                if board.is_attacked_by(not side, sq) and not board.is_attacked_by(side, sq):
                    count += 1
        return float(count)
            ...
            for sq in chess.SQUARES:
                ...
                if piece and piece.color == opp:
                    ...
                    if not defenders and attackers:
                        ...
                        if opp_king_sq is not None and chess.square_distance(sq, opp_king_sq) <= 1:
                            near_king = 1
    ...
    def king_ring_pressure_real(board, attacker):
        ...
        weight = {chess.PAWN:1.0, chess.KNIGHT:3**0.7, chess.BISHOP:3.1**0.7,
                  chess.ROOK:5**0.7, chess.QUEEN:9**0.7}
        s = 0.0
        ...
        for sq in ring:
            if board.is_attacked_by(attacker, sq):
                ...
                for pt, w in weight.items():
                    if any(board.piece_type_at(x) == pt and board.color_at(x) == attacker
                           for x in board.attackers(attacker, sq)):
                        s += w
                        break

        # Normalize by phase
        phase = sum(len(board.pieces(pt, True)) + len(board.pieces(pt, False))
...
def sf_eval(engine, board: "chess.Board", cfg: SFConfig) -> float:
    limit = chess.engine.Limit(depth=cfg.depth) if cfg.movetime==0 else chess.engine.Limit(time=cfg.movetime/1000.0)
...
def sf_top_moves(engine, board: "chess.Board", cfg: SFConfig) -> List[Tuple[chess.Move, float]]:
    limit = chess.engine.Limit(depth=cfg.depth) if cfg.movetime==0 else chess.engine.Limit(time=cfg.movetime/1000.0)
...
# Position sampling
# -----------------------------
def sample_positions_from_pgn(path: str, max_positions: int, ply_skip: int = 8) -> List["chess.Board"]:
    boards = []
...
PIECE_VAL = {chess.PAWN:1, chess.KNIGHT:3, chess.BISHOP:3.1, chess.ROOK:5, chess.QUEEN:9}

def _rank_distance(pawn_sq: int, color: bool) -> int:
    r = chess.square_rank(pawn_sq)
    return (7 - r) if color == chess.WHITE else r  # plies to 8th/1st rank in ranks, not moves

def _rook_behind_passer(board: chess.Board, pawn_sq: int, color: bool) -> int:
    file_idx = chess.square_file(pawn_sq)
    ranks = range(chess.square_rank(pawn_sq)-1, -1, -1) if color==chess.WHITE else range(chess.square_rank(pawn_sq)+1, 8)
    for r in ranks:
...
def _runner_clear_path(board: chess.Board, pawn_sq: int, color: bool) -> int:
    ...
    return int(_stoppers_in_path(board, pawn_sq, color) == 0 and _blockaded(board, pawn_sq, color) == 0)
...
def passed_pawn_momentum_snapshot(board: chess.Board, color: bool):
    ...
    for sq in board.pieces(chess.PAWN, color):
        if not _is_passed_pawn(board, sq, color):
            continue
        features["pp_count"] += 1
        features["pp_min_dist"] = min(features["pp_min_dist"], _rank_distance(sq, color))
...
def passed_pawn_momentum_delta(base: chess.Board, after_reply: chess.Board):
    ...
    def d(key): 
        return (after_us[key]-base_us[key]) - (after_them[key]-base_them[key])  # favor our gain vs their gain

    return {
        "d_pp_count":         d("pp_count"),
        "d_pp_min_dist":      -(after_us["pp_min_dist"] - base_us["pp_min_dist"]) + (after_them["pp_min_dist"] - base_them["pp_min_dist"]),
        "d_pp_runners_clear": d("pp_runners_clear"),
        "d_pp_blockaded":     -(after_us["pp_blockaded"] - base_us["pp_blockaded"]) + (after_them["pp_blockaded"] - base_them["pp_blockaded"]),
...
def confinement_delta(base: chess.Board, after_reply: chess.Board):
    us = base.turn
    return {
        "d_confinement": (confinement_count(after_reply, not us) - confinement_count(base, not us)) -
...
def audit_feature_set(
    ...
    for b in tqdm(boards, desc="Move delta collection"):
        ...
        for move, _ in top_moves:
            ...
            if reply_move is not None:
                ...
                after_reply_feats = {k: (1.0 if isinstance(v, bool) and v else float(v)) for k, v in after_reply_feats.items()}
                
                # Add engine-based features for training
                if after_reply_probes:
                    # Add hanging after reply features
                    hang_cnt, hang_max_val, hang_near_king = after_reply_probes['hanging_after_reply'](engine, b, depth=6)
                    ...
                    # Add forcing swing
                    forcing_swing = after_reply_probes['best_forcing_swing'](engine, b, d_base=6, k_max=12)
                ...
                check_delta = {
                    ...
                    "d_capture_checks": after_check["d_capture_checks"] - base_check["d_capture_checks"]
                }
    ...
    X_mat = np.array([[float(x.get(k, 0.0)) for k in feature_names] for x in X], dtype=float)
    y = np.array(y, dtype=float)

    # Train/test split on move deltas
    X_train, X_test, y_train, y_test = train_test_split(X_mat, y, test_size=test_size, random_state=42)
    ...
    model = LassoCV(cv=min(5, X_train.shape[0]//10), random_state=42, max_iter=10000, alphas=alphas)
    ...
    if model.alpha_ < 1.0:
        model.alpha_ = 1.0
        model = Lasso(alpha=model.alpha_, fit_intercept=True, random_state=42, max_iter=10000)
        model.fit(X_train_scaled, y_train)
    ...
    for b in tqdm(B_test, desc="Move ranking analysis"):
        # Get engine top-k
        cfg_local = SFConfig(cfg.engine_path, cfg.depth, cfg.movetime, multipv=min(multipv_for_ranking, cfg.multipv), threads=cfg.threads)
        cand = sf_top_moves(engine, b, cfg_local)
        if len(cand) < 2:
            continue
        # Build surrogate scores by evaluating features AFTER each candidate move
        base_feats = extract_features_fn(b)
        base_vec = np.array([float(base_feats.get(k, 0.0)) for k in feature_names], dtype=float)
        sf_scores = []
        sur_scores = []
        ranks = list(range(len(cand)))
        ...
        for mv, _ in cand:
            ...
            if reply_move is not None:
                ...
                feats_after_reply = {k: (1.0 if isinstance(v, bool) and v else float(v)) for k, v in feats_after_reply.items()}
                ...
                vec_after_reply = np.array([float(feats_after_reply.get(k, 0.0)) for k in feature_names], dtype=float)
                
                # Use linear model on after-reply features (delta training)
                vec_after_reply_scaled = scaler.transform(vec_after_reply.reshape(1, -1))
    ...
    weight_threshold = np.percentile(abs_coef[abs_coef > 0], 50) if np.any(abs_coef > 0) else 0.0
    ...
    for b in tqdm(B_test, desc="Faithfulness analysis"):
        ...
        if reply_move is not None:
            ...
            f_best = {k: (1.0 if isinstance(v, bool) and v else float(v)) for k, v in f_best.items()}
            ...
            vec_best = np.array([float(f_best.get(k, 0.0)) for k in feature_names], dtype=float)
            b.pop()
        else:
            delta_sf_best = 0.0
        ...
        if reply_move is not None:
            ...
            f_second = {k: (1.0 if isinstance(v, bool) and v else float(v)) for k, v in f_second.items()}
            ...
            vec_second = np.array([float(f_second.get(k, 0.0)) for k in feature_names], dtype=float)
            b.pop()
        else:
            delta_sf_second = 0.0
        ...
        strong_feats = np.sum((np.abs(coef) >= weight_threshold) & (np.abs(contrib_best) > 0))
        coverage_total += 1
        if strong_feats >= 2:
            coverage_hits += 1
    ...
    local_faithfulness_decisive = (faithful_decisive_hits / faithful_decisive_total) if faithful_decisive_total else 0.0
    ...
    if X_train.shape[0] >= 20:
        ...
        for b in tqdm(range(stability_bootstraps), desc="Stability selection"):
            ...
            yb = y_train[idx]
            m = Lasso(alpha=model.alpha_, fit_intercept=True, random_state=42+b, max_iter=10000)
            m.fit(Xb, yb)
            picks += (m.coef_ != 0.0)
    ...
    top_features = sorted([(feature_names[i], float(coef[i])) for i in range(len(feature_names))], key=lambda x: -abs(x[1]))[:15]
...
def main():
    ap = argparse.ArgumentParser(description="Audit explainability of a chess feature set against Stockfish.")
    ap.add_argument("--engine", type=str, default=os.environ.get("STOCKFISH_PATH", ""), help="Path to Stockfish binary")
    ap.add_argument("--threads", type=int, default=1)
    ap.add_argument("--depth", type=int, default=16, help="Fixed search depth (set 0 to use movetime)")
    ap.add_argument("--movetime", type=int, default=0, help="Milliseconds per evaluation if depth=0")
    ap.add_argument("--multipv", type=int, default=3, help="Max MultiPV used for ranking metric")
    ap.add_argument("--positions", type=int, default=400)
    ap.add_argument("--pgn", type=str, default="", help="Optional PGN file to sample positions")
    ap.add_argument("--ply-skip", type=int, default=8, help="Keep every Nth ply when sampling from PGN")
    ap.add_argument("--features_module", type=str, default="", help="Path to a Python module that defines extract_features(board)")
    ap.add_argument("--baseline_features", action="store_true", help="Use built-in small baseline features")
    ap.add_argument("--test_size", type=float, default=0.25)
    ap.add_argument("--alpha", type=float, default=0.01, help="L1 regularization strength for surrogate")
    ap.add_argument("--gap", type=float, default=50.0, help="CP gap to treat best vs second as decisive for faithfulness")
    ...
    if not args.engine:
        print("Please provide --engine PATH or set STOCKFISH_PATH env var.", file=sys.stderr)
        sys.exit(1)

    cfg = SFConfig(engine_path=args.engine, depth=args.depth if args.depth>0 else 0, movetime=args.movetime if args.depth==0 else 0, multipv=max(2, args.multipv), threads=args.threads)

    # Positions
    if args.pgn:
        boards = sample_positions_from_pgn(args.pgn, args.positions, ply_skip=args.ply_skip)
    ...
    if args.baseline_features:
        ...
        print("Provide --features_module PATH or use --baseline_features to run.", file=sys.stderr)
        sys.exit(1)
    ...
    print(f"Positions: {len(boards)}  |  Depth: {cfg.depth or 'movetime'}  |  MultiPV: {cfg.multipv}")
    print(f"Fidelity (Delta-R^2):          {res.r2:0.3f}")
    print(f"Move ranking (Kendall tau):    {res.tau_mean:0.3f}  (positions covered: {res.tau_covered}/{res.n_tau})")
    print(f"Local faithfulness (best vs 2): {res.local_faithfulness*100:0.1f}% (gap ≥ {args.gap} cp)")
    print(f"Local faithfulness (decisive): {res.local_faithfulness_decisive*100:0.1f}% (gap ≥ 80.0 cp)")
    print(f"Sparsity (reasons to cover 80% contribution for best move): {res.sparsity_mean:0.2f}")
    ...
    if res.stable_features:
        print("\nStable features (picked ≥{:.0f}% of bootstraps):".format(100*0.7))
    ...
    print("\nGuidance:")
    print(" - Aim for Delta-R^2 ≥ 0.60 on mixed middlegames at depth ~16 as a healthy baseline.")
    print(" - Tau ≥ 0.45 for top-3 move ranking is decent; higher is better.")
    print(" - Local faithfulness ≥ 80% on decisive positions shows explanations track preferences.")
    print(" - Sparsity around 3–5 suggests crisp, narratable reasons.")
    print(" - Coverage ≥ 70% with ≥2 strong reasons means you can explain most positions.\n")
